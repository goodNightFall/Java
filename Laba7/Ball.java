...
/***
 * В классе BallAnimation, где реализован интерфейс Runnable, запускается
 * отдельный поток для выполнения анимации. В методе run() этого потока
 * происходит обновление координат шарика и его перерисовка на панели.
 * 
 * Это позволяет обеспечить беспрерывное перемещение шарика в отдельном потоке,
 * не блокируя главный поток выполнения программы.
 ***/
