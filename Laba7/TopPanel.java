...
public class TopPanel extends JPanel implements Runnable {
  ...
  TopPanel(RestartButton restartButton, Score score) {
    ...
    this.setBorder(
        BorderFactory.createEmptyBorder((this.getHeight() - restartButton.getHeight() - score.getHeight()) / 2,
            Main.GAME_WIDTH / 2 - score.getValueLength() * 10, 0, 0));
  ...
  public void run() {
    while (isThreadRunning) {
      ...
      score.setBorder(BorderFactory.createEmptyBorder(0,
          (this.restartButton.getWidth() - this.score.getValueLength() * 10) / 2, 0, 0));
      score.repaint();
    }
  }
